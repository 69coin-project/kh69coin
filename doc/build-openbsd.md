...
The default C++ compiler that comes with OpenBSD 6.2 is g++ 4.2.1. This version is old (from 2007), and is not able to compile the current version of Kh69coin Core because it has no C++11 support. We'll install a newer version of GCC:
...
```bash
# Pick some path to install BDB to, here we create a directory within the kh69coin directory
KH69COIN_ROOT=$(pwd)
BDB_PREFIX="${KH69COIN_ROOT}/db4"
mkdir -p $BDB_PREFIX
