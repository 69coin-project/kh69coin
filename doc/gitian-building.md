...
*Setup instructions for a Gitian build of Kh69coin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Kh69coin
...
- [Building Kh69coin Core](#building-kh69coin-core)
...
Clone the git repositories for kh69coin and Gitian.
...
in the kh69coin repository under 'Fetch and create inputs' to install sources which require
...
To build Kh69coin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the kh69coin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kh69coin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/kh69coin-project/kh69coin
...
For example:
```bash
URL=https://github.com/thrasher-/kh69coin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kh69coin=${COMMIT} --url kh69coin=${URL} ../kh69coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kh69coin=${COMMIT} --url kh69coin=${URL} ../kh69coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kh69coin=${COMMIT} --url kh69coin=${URL} ../kh69coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the kh69coin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kh69coin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/kh69coin
SIGPATH=/some/root/path/kh69coin-detached-sigs

./bin/gbuild --url kh69coin=${BTCPATH},signature=${SIGPATH} ../kh69coin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kh69coin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kh69coin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kh69coin-osx-build.assert
```
...
[kh69coin-project/gitian.sigs.ltc](https://github.com/kh69coin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
